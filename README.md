# CloudBlitz App — Course Enrollment Platform
...
| Region | `eu-north-1` |
| Frontend URL | `https://www.thedevopsmaster.online` |
| API URL | `https://api.thedevopsmaster.online` |
| EKS cluster | `cdec-eks-dev` |
| Terraform state bucket | `cdec-alpha-terraform-state-atulyw` |

> Update the AWS account, EKS cluster name, and Terraform state bucket above if they differ in your environment — only the domain and region were confirmed as changed.
...
flowchart TB
    ...
    subgraph Frontend["Frontend (S3 + CloudFront)"]
        SPA[React SPA<br/>www.thedevopsmaster.online]
    end

    subgraph API["API Gateway (ALB Ingress on EKS)"]
        ALB[Application Load Balancer<br/>api.thedevopsmaster.online]
    end
...
sequenceDiagram
    ...
    User->>SPA: Open www.thedevopsmaster.online
...
|----------|------------------|
| `VITE_AUTH_API` | `https://api.thedevopsmaster.online/api/auth` |
| `VITE_COURSE_API` | `https://api.thedevopsmaster.online/api/courses` |
| `VITE_ENROLL_API` | `https://api.thedevopsmaster.online/api/enroll` |
...
- CloudFront distribution (ACM cert in `us-east-1` — CloudFront requires this regardless of your app region)
- Route 53 hosted zone / DNS alias for `www.thedevopsmaster.online`
...
export VITE_AUTH_API=https://api.thedevopsmaster.online/api/auth
export VITE_COURSE_API=https://api.thedevopsmaster.online/api/courses
export VITE_ENROLL_API=https://api.thedevopsmaster.online/api/enroll
npm run build
...
ingress_host       = "api.thedevopsmaster.online"
include_caller_as_cluster_admin = true   # grants Jenkins/Terraform user kubectl access
```

> **Prerequisite:** Create ECR repositories and an ACM certificate for `api.thedevopsmaster.online` in `eu-north-1` before deploying services.
...
```text
933516006319.dkr.ecr.eu-north-1.amazonaws.com/backend/<service-name>:latest
```
...
docker build -t 933516006319.dkr.ecr.eu-north-1.amazonaws.com/backend/auth-service:latest .

aws ecr get-login-password --region eu-north-1 | \
  docker login --username AWS --password-stdin 933516006319.dkr.ecr.eu-north-1.amazonaws.com
docker push 933516006319.dkr.ecr.eu-north-1.amazonaws.com/backend/auth-service:latest

aws eks update-kubeconfig --region eu-north-1 --name cdec-eks-dev
...
```bash
# Frontend
curl -I https://www.thedevopsmaster.online

# API health endpoints
curl https://api.thedevopsmaster.online/api/auth/health
curl https://api.thedevopsmaster.online/api/courses/health
curl https://api.thedevopsmaster.online/api/enroll/health
